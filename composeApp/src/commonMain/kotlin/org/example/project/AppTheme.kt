...
import kmpproject.composeapp.generated.resources.notosansjp_regular
import org.jetbrains.compose.resources.Font

// ライトテーマ（標準コントラスト）。Material Theme Builder が生成した定義を保持する。
...
// ライトテーマ（中コントラスト）。視認性向上が必要な場合に利用するため保持。
...
/**
 * カラーファミリーのまとまり。色と対になる onColor / Container 系を一括で扱うための小さなDTO。
 *
 * 利用例:
 * ```
 * val primaryFamily = ColorFamily(primaryLight, onPrimaryLight, primaryContainerLight, onPrimaryContainerLight)
 * val textColor = primaryFamily.onColor // コンテナ上で読みやすいテキスト色を取得
 * ```
 */
@Immutable
...
/**
 * アプリで利用するThemeを定義する。
 * ダークモードをシステム設定から検出し、共通フォント(Noto Sans JP)とともに MaterialTheme に適用する。
 *
 * 利用例:
 * ```
 * @Composable
 * fun Screen() {
 *     AppTheme {
 *         // ここに配置したUIはアプリ共通の色・タイポグラフィが適用される
 *         Content()
 *     }
 * }
 * ```
